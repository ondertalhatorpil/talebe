...
class Quiz {
  static async createQuizByCategory(userId, categoryId, questionCount = 10) {
    try {
      ...
      const limitCheck = await User.checkDailyLimit(userId, categoryId);
      ...
      const [recentlyAnsweredRows] = await pool.execute(
        `SELECT DISTINCT question_id 
         ...
         AND answered_at > ?`,
        [userId, categoryId, oneWeekAgo]
      );
      
      const recentlyAnsweredQuestionIds = recentlyAnsweredRows.map(row => row.question_id);
      
      // Maksimum soru sayısı kontrolü
      const availableQuestionCount = Math.min(questionCount, limitCheck.remaining);
      
      // ✅ DÜZELTİLDİ: Boş IN clause kontrolü eklendi
      let query = `
      ...
      // ✅ DÜZELTME: Sadece recent IDs varsa hariç tut
      ...
      params.push(availableQuestionCount);
      ...
      if (questions.length < availableQuestionCount && recentlyAnsweredQuestionIds.length > 0) {
        const remainingCount = availableQuestionCount - questions.length;
        
        // ✅ DÜZELTME: placeholders ile IN clause oluştur
        const placeholders = recentlyAnsweredQuestionIds.map(() => '?').join(',');
        ...
        const [oldQuestions] = await pool.execute(
          `SELECT id, question_text, difficulty, points 
           ...
           ORDER BY RAND() 
           LIMIT ?`,
          [categoryId, ...recentlyAnsweredQuestionIds, remainingCount]
        );
        
        questions = [...questions, ...oldQuestions];
      }
      
      // ✅ KONTROL: Eğer hiç soru yoksa hata dön
      ...
      console.error('Quiz creation error:', error); // Debug için
      throw error;
    }
  }
